...
<style>
.main {{
background-image:url("https://th.bing.com/th/id/OIP.NFW-zooxcfZwo2JtUYFuVgHaEK?pid=ImgDet&rs=1");
...
class Detection:
    ...
    def __call__(self):
        ...
        st.title("\U0001F4F8 :red[Lets Detect the objects]")
        ...
        if st.sidebar.checkbox('Using Camera',key="camera is used"):
            ...
            if st.sidebar.checkbox('Upload the images',key="file"):
               uploaded_file = st.file_uploader(":orange[_**CHOOSE A FILE**_]",type=['jpg','png','jpeg'])
...
detector = Detection(capture_index=0, model_name="D:\\Tata-Motors\\last.pt") 
detector()
